...
/****************************************************************************
 * (c) Neuroscan $Date: 2020-05-08 17:54:23 +0200 (Fri, 08 May 2020) $
 *
 * $Revision: 23882 $
 * $Author: tech $
 **************************************************(header generated by SVN)*/
/****************************************************************************/
// static char* REV = "$Revision: 23882 $";
// static char* DAT = "$Date: 2020-05-08 17:54:23 +0200 (Fri, 08 May 2020) $";
...
// void RNetStreamingReceiver::AddLogLine(string sLogLine, long nLogID)
// {
// 	if (m_pAddLogLine)
// 	{
...
// BOOL RNetStreamingReceiver::StartClientThread()
// {
// 	string sError;
...
{
	// std::cout << "I am in OpenClient()" << std::endl;
    ...
    // else
	// {
    //    AddLogLine("Client: socket() is OK.");
	// }
	...
	if (connect(m_SocketClient, (struct sockaddr*)&server, sizeof(server)) == SOCKET_ERROR)
	{
		...
		// else
		// {
			// AddLogLine("Client: connect() is OK.");
		// }
    }
    else
	{
...
{
	// std::cout << "I am in SendNetStreamingMessage" << std::endl;
	// printf("ID%ld, word%ld, dword%ld\n", sizeof(pMessage->m_chId), sizeof(WORD), sizeof(DWORD));
	string s;
	...
	if (!pMessage->m_pBody)
	{
		// std::cout << "I am in pbody" << std::endl;
		// ASSERT(pMessage->m_dwSize == 0);
		pMessage->m_dwSize = 0;
	}
	// std::cout << "m_dwSize is " << pMessage->m_dwSize << std::endl;
	DWORD dwSize = pMessage->m_dwSize;													//copy size because function Convert() will change m_dwSize variable
	// std::cout << "header size is " << pMessage->GetHeaderSize() << std::endl;
	// std::cout << "ch ID is " << pMessage->m_chId << std::endl;
...
// HRESULT RNetStreamingReceiver::SendHTTPMessage(SOCKET socket, string sHTTP)
// {
...
{
	// std::cout << "I am in ReceiveNetStreamingMessage" << std::endl;
	int		error			= 0;
	int		nRetval			= 0;
	...
	// std::cout << "ch ID is " << pMessage->m_chId << std::endl;
	// std::cout << "nRetval is " << nRetval << std::endl;
	...
	// std::cout << "is valid? " << pMessage->IsValid() << std::endl;
	...
	// std::cout << "m_dwSize: " << pMessage->m_dwSize << std::endl;
...
{
	// std::cout << "I am in HandleMessage" << std::endl;
	HRESULT		hr				= S_OK;
			...
			case DATA_Info:
				switch (pMessage->m_wRequest)
				{
					case InfoType_Version:
						{
							// std::cout << "I am in InfoType_Version" << std::endl;
							// std::cout << "m_dwSize is " << pMessage->m_dwSize << std::endl;
							// std::cout << "m_nNetStreamingVersion is " << m_nNetStreamingVersion << std::endl;
							// std::cout << "size of m_nNetStreamingVersion is " << sizeof(m_nNetStreamingVersion) << std::endl;
							...
							// std::cout << "message handle is " << bMessageHandled << std::endl;
						}
						break;
...
// HRESULT RNetStreamingReceiver::ClientRequestSetRecPath(string sRecPath)
// {
...
// 	AcqMessage message(NETSTREAM_CTRL, CTRL_FromClient, Request_SetRecPath);
	
// 	wchar_t pwTmp[MAX_PATH];
// 	swprintf_s(pwTmp, L"%s", sRecPath.GetString());		//copy string to wchar_t array
...
// HRESULT RNetStreamingReceiver::ClientRequestControlAmp(bool bStart)
// {
// 	if (!m_bClientInited)
// 	{
// 		return E_FAIL;
...
// 	AcqMessage message(NETSTREAM_CTRL, CTRL_FromClient, bStart ? Request_AmpConnect : Request_AmpDisconnect);
// 	return SendNetStreamingMessage(m_SocketClient, &message);
// }

// //asks server to start or stop recording (needs to be allowed by server)
// HRESULT RNetStreamingReceiver::ClientRequestControlRecording(bool bStart)
// {
// 	if (!m_bClientInited)
// 	{
// 		return E_FAIL;
...
// 	AcqMessage message(NETSTREAM_CTRL, CTRL_FromClient, bStart ? Request_RecordingStart : Request_RecordingStop);
// 	return SendNetStreamingMessage(m_SocketClient, &message);
// }

// //asks server to start or stop impedance test (needs to be allowed by server)
// HRESULT RNetStreamingReceiver::ClientRequestControlImpedanceTest(bool bStart)
// {
// 	if (!m_bClientInited)
// 	{
// 		return E_FAIL;
...
// 	AcqMessage message(NETSTREAM_CTRL, CTRL_FromClient, bStart ? Request_ImpedanceStart : Request_ImpedanceStop);
// 	return SendNetStreamingMessage(m_SocketClient, &message);
// }
...
// HRESULT RNetStreamingReceiver::ClientGetDelay(double* pdDelayMS)
// {
...
// 	//send basic info request to server
// 	AcqMessage message(NETSTREAM_CTRL, CTRL_FromClient, Request_Delay);
// 	if (FAILED(SendNetStreamingMessage(m_SocketClient, &message)))
// 	{
// 		ASSERT(false);
// 		return E_FAIL;
